...
class UserSyncEventPublisher(private val kafkaTemplate: KafkaTemplate<String, String>) {
    ...
    fun publish(userId: Long, username: String) {
        kafkaTemplate.send("user-sync", userId.toString(), """{"userId":$userId,"username":"$username"}""")
            .whenComplete { _, ex ->
    ...
    fun publishUsernameUpdate(userId: Long, newUsername: String) {
        kafkaTemplate.send("user.username.updated", userId.toString(), """{"userId":$userId,"newUsername":"$newUsername"}""")
            .whenComplete { _, ex ->
