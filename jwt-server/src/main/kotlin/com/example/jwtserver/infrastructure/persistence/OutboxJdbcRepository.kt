...
class OutboxJdbcRepository(private val jdbcTemplate: JdbcTemplate) : OutboxRepository {
    ...
    override fun markSent(id: Long) {
        jdbcTemplate.update(
            "UPDATE jwt_db.public.outbox_events SET sent_at = NOW() WHERE id = ?", id
        )
    }

    override fun unclaim(id: Long) {
        jdbcTemplate.update(
            "UPDATE jwt_db.public.outbox_events SET claimed_at = NULL WHERE id = ?", id
        )
    }
    ...
    override fun resetStaleClaims() {
        jdbcTemplate.update(
            """
            UPDATE jwt_db.public.outbox_events SET claimed_at = NULL
    ...
    override fun deleteProcessed() {
        jdbcTemplate.update(
            "DELETE FROM jwt_db.public.outbox_events WHERE sent_at IS NOT NULL AND sent_at < NOW() - INTERVAL '7 days'"
        )
    }
